...
- **App tab** — select apps to install: Brave Origin Beta/Stable, Visual Studio Code, VS Code Insiders, Neovim, Alacritty, WezTerm, Antigravity IDE\*, Claude Code CLI, Antigravity 2.0\*, Antigravity CLI, Codex CLI, Cursor CLI, Opencode CLI, Kimi Code CLI, Muse, Hermes Agent, Devin CLI, Grok CLI, Mistral CLI, Postman CLI, Bun, Vicinae\*, Notion\*, Obsidian\*, VLC, OBS Studio, Zoom\*, Flatpak, Docker + Compose, Claude Desktop† (\*Arch only, †Debian/Ubuntu only — see below). OBS Studio pulls in `v4l2loopback-dkms` (virtual camera) and `qt6-wayland` (Wayland rendering) with it — both are required for it to work, not optional extras
