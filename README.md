...
`rofi` (Arch ships 2.0 with Wayland support merged in; Debian/Ubuntu are still on 1.7.x X11-only), `notion` (no official Linux build, only unofficial wrappers exist), `obsidian` (in Arch `extra`; on Debian/Ubuntu it ships only as a vendor `.deb`/AppImage with no apt repo), the Antigravity desktop app / IDE (Google's Debian/Ubuntu packaging is still a moving target upstream), `vicinae` (AUR only: upstream ships a release tarball and a Nix flake, no apt repo), `zoom` (a vendor `.deb` behind a download page, no apt repo) and `deepseek-harness` (AUR only, `deepseek-harness-bin`; DeepSeek's plugin-based agent harness — `dsh web` opens its browser UI) are only offered on Arch. Picking Vicinae also enables its `vicinae.service` user unit, and starts it when there is already a session to draw on: `vicinae toggle` (the command to bind to a hotkey) is an IPC call into that daemon, so it does nothing until it runs. `antigravity-cli` is available everywhere via its official install script.
